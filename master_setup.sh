#!/usr/bin/env bash
# ============= master_setup.sh =============
# Главный скрипт для полного развертывания системы
# ===========================================
set -euo pipefail

# Цвета для вывода
GREEN='\033[0;32m'
BLUE='\033[0;36m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m'

info() { echo -e "${BLUE}[i]${NC} $1"; }
log()  { echo -e "${GREEN}[✓]${NC} $1"; }
warn() { echo -e "${YELLOW}[!]${NC} $1"; }
err()  { echo -e "${RED}[✗]${NC} $1"; exit 1; }

# Обработка ошибок
error_handler() {
    local exit_code=$?
    local line_number=$1
    local command="$2"
    echo -e "${RED}[✗] Ошибка в строке $line_number: команда '$command' завершилась с кодом $exit_code${NC}"
    exit $exit_code
}
trap 'error_handler ${LINENO} "$BASH_COMMAND"' ERR

# Проверка на root
if [[ $EUID -ne 0 ]]; then
   err "Этот скрипт должен выполняться от имени root"
fi

PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$PROJECT_DIR"

clear
echo -e "${BLUE}"
echo "====================================================="
echo "   🚀 VPN AUTO-INSTALLER: ALL-IN-ONE SETUP"
echo "====================================================="
echo -e "${NC}"

# 1. Настройка прав и проверка .env
info "Проверка конфигурации..."
chmod +x scripts/*.sh

if [[ ! -f ".env" ]]; then
    info "Файл .env не найден. Создаю из .env.example..."
    cp .env.example .env
    warn "Файл .env создан. Пожалуйста, отредактируйте его (nano .env) и запустите скрипт снова."
    exit 0
fi

# Запуск жесткой проверки
if ! ./scripts/00-pre-check.sh; then
    exit 1
fi

log "Конфигурация проверена."

info "Установка Python зависимостей..."
apt-get install -y python3-pip python3-venv >/dev/null 2>&1 || true
python3 -m pip install --break-system-packages -r scripts/bot/requirements.txt >/dev/null 2>&1 || python3 -m pip install -r scripts/bot/requirements.txt >/dev/null 2>&1
log "Python зависимости установлены."


# 3. Базовая подготовка сервера
info "Шаг 1/5: Подготовка сервера и безопасность..."
./scripts/01-init-server.sh
log "Сервер подготовлен."

# 4. Установка Docker
info "Шаг 2/5: Установка Docker..."
./scripts/02-install-docker.sh
log "Docker установлен."

# 5. Деплой контейнеров
info "Шаг 3/5: Запуск контейнеров (VPN, Warp, AdGuard)..."
./scripts/03-deploy.sh --no-prompt
log "Контейнеры запущены."

# 5.5 Автоматическое обновление GeoData
info "Шаг 3.5/5: Настройка автообновления GeoData (маршрутизация РФ)..."
python3 ./scripts/bot/vpn_manager.py --update-geodata 2>/dev/null || warn "Сбой при первом скачивании GeoData. Крон настроен."

if ! crontab -l 2>/dev/null | grep -q "vpn_manager.py --update-geodata"; then
    (crontab -l 2>/dev/null || true; echo "0 3 * * * /usr/bin/python3 $PROJECT_DIR/scripts/bot/vpn_manager.py --update-geodata >/dev/null 2>&1") | crontab -
    log "Настроен cron для ежедневного автообновления GeoData."
fi

# 6. Настройка Telegram Бота (если есть токен)
source .env
if [[ -n "${TG_BOT_TOKEN:-}" ]]; then
    info "Шаг 4/5: Настройка мониторинга..."
    ./scripts/09-setup-bot.sh
    log "Мониторинг настроен."
else
    warn "TG_BOT_TOKEN не указан. Пропускаю настройку мониторинга."
fi

# 7. Авто-настройка Панели и AdGuard
info "Шаг 5/5: Финальная настройка панелей..."
python3 ./scripts/bot/vpn_manager.py --setup-inbound
./scripts/10-setup-adguard.sh
log "Панели настроены."

echo -e "${GREEN}"
echo "====================================================="
echo "   ✨ УСТАНОВКА ЗАВЕРШЕНА УСПЕШНО! ✨"
echo "====================================================="
echo -e "${NC}"
info "Ваш VPN готов к работе."
info "Выполните python3 ./scripts/bot/vpn_manager.py --show-clients для получения ссылок."
