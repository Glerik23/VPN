...
FREE_SPACE=$(df -m "$BACKUP_DIR" | tail -1 | awk '{print $4}')
if [[ "$FREE_SPACE" -lt 500 ]]; then
    warn "Мало места на диске (${FREE_SPACE}MB). Бэкап может не удаться."
fi

mkdir -p "$BACKUP_PATH"

# Сохраняем список образов (полезно для восстановления без интернета)
docker images --format "{{.Repository}}:{{.Tag}}" > "$BACKUP_PATH/docker-images.txt"
